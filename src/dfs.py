...
def dfs(initial_state, goal_state):
    ...
    while stack:
        pointer_node = stack.pop()

        # if we start with goal state just end.
        ...
        if tuple(map(tuple, pointer_node.state)) not in viewed:
            ...
            for travel in travels:
                ...
                if 0 <= new_row < 3 and 0 <= new_col < 3:
                    ...
                    stack.append(Node(newest_state, pointer_node, travel, pointer_node.cost + 1))
...
def run_algorithm(result_textbox, runs_entry, treeview):
    ...
    for i in range(N):
        ...
        algorithm_result, nodes_viewed = dfs(initial_state, goal_state)
        end_time = time.time()
...
def create_gui():
    root = tk.Tk()
    # program title
    root.title("DFS 8 Puzzle Solver")
