...
def ucs(initial_state, goal_state):
    viewed = set()
    nodes_vis = 0
    priority_queue = [Node(initial_state)]
        ...
        if tuple(map(tuple, pointer_node.state)) not in viewed:
            ...
            for travel in travels:
                ...
                if 0 <= new_row < 3 and 0 <= new_col < 3:
                    ...
                    heapq.heappush(priority_queue, Node(newest_state, pointer_node, travel, pointer_node.cost + 1))
...
def run_algorithm(result_textbox, runs_entry, treeview):
    ...
    for i in range(N):
        ...
        algorithm_result, nodes_viewed = ucs(initial_state, goal_state)
        end_time = time.time()
...
def create_gui():
    root = tk.Tk()
    # program title
    root.title("UCS 8 Puzzle Solver")
    ...
    # start button
    #run_button = ttk.Button(root, text="Run algorithm", command=partial(run_al, 1, runs_entry, result_textbox))
