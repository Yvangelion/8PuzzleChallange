...
def bfs(initial_state, goal_state):
    ...
    while queue:
        ...
        if tuple(map(tuple, pointer_node.state)) not in viewed:
            ...
            # view the held node and enqueue the unviewed child nodes
            ...
            for travel in travels:
                ...
                if 0 <= new_row < 3 and 0 <= new_col < 3:
                    ...
                    queue.append(Node(newest_state, pointer_node, travel, pointer_node.cost + 1))
...
def run_algorithm(result_textbox, runs_entry, treeview):
    ...
    for i in range(N):
        ...
        algorithm_result, nodes_viewed = bfs(initial_state, goal_state)
        end_time = time.time()
...
def create_gui():
    root = tk.Tk()
    # program title
    root.title("BFS 8 Puzzle Solver")
